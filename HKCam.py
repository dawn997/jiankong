...
if __name__=="__main__":
    ...
    while True:
        ...
        try:
            ...
            response = client.detect_pedestrian_advance(detect_pedestrian_request, runtime)
            # 获取整体结果
            print(response.body)
            # print(response.body.__dict__['data'].elements[0].type)
            # if response.body.__dict__['data'].elements[0].type != None:
            if len(response.body.__dict__['data'].elements) != 0:
                ssh_scpHtmlfile(pic_name)
                print("存在人类")
                # send_mail("有人出现")
        ...
        # cv2.imshow('beizui',cv2.resize(img,(960,540)))
        kkk = cv2.waitKey(1)
        if kkk ==ord('q'):
            break
    hkclass.release()
